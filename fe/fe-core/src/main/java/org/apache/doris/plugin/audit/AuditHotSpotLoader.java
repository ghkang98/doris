...
public class AuditHotSpotLoader extends Plugin implements AuditPlugin {
    ...
    public boolean eventFilter(AuditEvent.EventType type) {
        return type == AuditEvent.EventType.AFTER_QUERY
            || type == AuditEvent.EventType.LOAD_SUCCEED
            || type == AuditEvent.EventType.STREAM_LOAD_FINISH;
    }
    ...
    private void fillLogBuffer(AuditEvent event, StringBuilder hotSpotBuffer) {
        if (event instanceof LoadAuditEvent) {
            handleLoadSuccess((LoadAuditEvent) event, hotSpotBuffer);
        } else if (event instanceof StreamLoadAuditEvent) {
            handleStreamLoadFinish((StreamLoadAuditEvent) event, hotSpotBuffer);
        } else {
            handleAfterQueryEvent(event, hotSpotBuffer);
        }
    }

    private void handleLoadSuccess(LoadAuditEvent event, StringBuilder hotSpotBuffer) {
        if (null == event.tableList) {
            return;
        }
        String[] tables = event.tableList.split(",");
        for (String table : tables) {
            String catalog = StringUtils.isEmpty(event.ctl) ? InternalCatalog.INTERNAL_CATALOG_NAME : event.ctl;
            hotSpotBuffer.append(catalog).append("\t");
            hotSpotBuffer.append(event.db).append("\t");
            hotSpotBuffer.append(table).append("\t");
    ...
    private void handleStreamLoadFinish(StreamLoadAuditEvent event, StringBuilder hotSpotBuffer) {
        String catalog = StringUtils.isEmpty(event.ctl) ? InternalCatalog.INTERNAL_CATALOG_NAME : event.ctl;
        hotSpotBuffer.append(catalog).append("\t");
        hotSpotBuffer.append(event.db).append("\t");
        hotSpotBuffer.append(event.table).append("\t");
        hotSpotBuffer.append(TimeUtils.longToTimeStringWithHour(event.timestamp)).append("\t");
        hotSpotBuffer.append(0).append("\t");
        hotSpotBuffer.append(1).append("\t");
        hotSpotBuffer.append(TimeUtils.longToTimeStringWithms(event.timestamp)).append("\t");
        hotSpotBuffer.append(1).append("\t");  //streamLoad write file num regard as 1
        hotSpotBuffer.append(event.loadedRows).append("\t");
        hotSpotBuffer.append(event.loadBytes).append("\t");
    ...
    private void handleAfterQueryEvent(AuditEvent event, StringBuilder hotSpotBuffer) {
        if (null == event.tableFullQualifiers) {
            return;
        }
